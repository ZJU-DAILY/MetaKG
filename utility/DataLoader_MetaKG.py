...
class DataLoader(object):
    ...
    def __init__(self, args, logging, state = 'meta_training'):
        self.args = args
        ...
        train_file = os.path.join(data_dir, 'test_scenario', state+'_support.txt')
        test_file = os.path.join(data_dir, 'test_scenario', state+'_query.txt')
        # train_file = os.path.join(data_dir, 'train.txt')
        # test_file = os.path.join(data_dir, 'test.txt')
        kg_file = os.path.join(data_dir, "kg_final.txt")

        print('train_miss_user_num: ')
        self.cf_train_data, self.train_user_dict = self.load_cf(train_file)  # (user, item), user_dict
        print('test_miss_user_num: ')
        self.cf_test_data, self.test_user_dict = self.load_cf(test_file)     # (user, item), user_dict
        self.statistic_cf()
        ...
        self.train_graph = self.create_graph(self.kg_train_data, self.n_users_entities)  # merge the "train.txt" and "kg_final.txt" with inverse triplets
        # self.test_graph = self.create_graph(self.kg_test_data, self.n_users_entities)

        # load pretrained user and item embedding
    ...
    def load_cf(self, filename):
        """
        generate user_item pair and user_dict
        """
        user = []
        item = []
        user_dict = dict()
    ...
    def statistic_cf(self):
        """
        compute the number of user, item and user_item pair
        """
        # self.n_users = max(max(self.cf_train_data[0]), max(self.cf_test_data[0])) + 1
        # self.n_items = max(max(self.cf_train_data[1]), max(self.cf_test_data[1])) + 1
        self.n_users = n_user_dataset[self.data_name]
        self.n_items = n_item_dataset[self.data_name]
    ...
    def load_kg(self, filename):
        """
        load triplets from "kg_final.txt"
        """
    ...
    def construct_data(self, kg_data):
        ...
        reverse_kg_data['r'] += n_relations  # inverse relation can't be regarded as the same relation
        kg_data = pd.concat([kg_data, reverse_kg_data], axis=0, ignore_index=True, sort=False)

        # re-map user id
        kg_data['r'] += 2   # the 0,1 need to present user-item and item-user (ones, zeros)
        self.n_relations = max(kg_data['r']) + 1
        self.n_entities = max(max(kg_data['h']), max(kg_data['t'])) + 1
        self.n_users_entities = self.n_users + self.n_entities
        
        # user_id = user_id + self.n_entities
        ...
        # merge the "train.txt" and "kg_final.txt" with inverse triplets
    ...
    def generate_cf_batch(self, user_dict, batch_user):
        '''
    ...
    def generate_kg_batch(self, kg_dict, batch_user):
        '''
